...
[예제 3-4]  결과에서 중복값 제외 
employees 테이블에서 중복 값이 생기지 않도록 job_id를
 출력하세요. 
...
[예제 3-5] 별칭 사용
employees 테이블에서 employee_id는  사원번호 ,
first_name은 이름, 
...
[예제 3-8] 산술처리
employees 테이블에서 employee_id, salary, 
salary에 500을 더한값, 
...
[예제 3-9] 산술처리
employees 테이블에서 employee_id를 사원번호, 
salary를 급여, 
salary에 500을 더한값을 추가급여, 
100을 뺀값을 인하급여, 
10%를 추가해서 2로 나눈 값을 조정급여로  출력 

SELECT employee_id 사원번호, salary 급여,
       salary+500 추가급여, salary-100 인하급여,
       (salary*1.1)/2 조정급여
  FROM employees;


[예제 3-10] WHERE 조건절 비교 연산자
employees 테이블에서 employee_id가 100인 
직원 정보를 출력
...
employees 테이블에서 first_name이 David인
 직원 정보를 출력
...
employees 테이블에서 employee_id가 105 이상인 
직원 정보를 출력
...
## 3-13이면서 입사월(hire_date)이 1월인 직원
...
employees 테이블에서 salary가 
10000, 17000, 24000인 직원 정보를 출력
...
## first_name이 Neena와 같은 salary를 받는 직원은?
...
employees 테이블에서 job_id값이 AD로 시작하는 
모든 데이터를 조회
...
employees 테이블에서 first_name이 Le로 
시작하는 모든 데이터를 조회
...
employees 테이블에서 first_name에 ame가 
들어있는 모든 데이터를 조회
...
employees 테이블에서 first_name이  IN 
끝나는 모든 데이터를 조회
...
employees 테이블에서 job_id가 AD로 시작하면서 
5자인 데이터 값을 갖는 직원 정보를 조회
...
employees 테이블에서 manager_id가 NULL 값인 
직원 정보를 출력

## 실습.. employees, manager_id null 인 데이터 3건을 등록
##  employee_id, last_name, email, hire_date, job_id 로 구성된 데이터를 
##  210, "a111","b111", "2021-07-03","DT"
##  211, "a222","b222", "2021-07-03","DT"
##  212, "a333","b333", "2021-07-03","DT"
...
INSERT INTO employees (employee_id, last_name, email, 
                             hire_date, job_id) 
     VALUES (211,'a222','b222','2021-07-03','dt');  
INSERT INTO employees (employee_id, last_name, email, 
                              hire_date, job_id) 
...
SELECT salary AS aa, first_name AS bb
  FROM employees
WHERE salary < 5000;
 
 
SELECT * 
  FROM ( SELECT salary AS aa, first_name AS bb
           FROM employees ) x
where  aa < 5000 ; 
...
SELECT job_id, REPLACE(job_id, 'FI_ACCOUNT', 'ACCNT') 
  FROM employees
...
employees 테이블에서 first_name에 대해 12자리의 
문자열 자리를 만들되 
first_name의 데이터 값이 12자리보다 작으면 왼쪽에서 
부터 *를 채워서 출력
...
employees 테이블에서 salary를 30일로 나눈 후
 나눈 값의 소수점 첫째 자리, 
소수점 둘째 자리, 정수 첫째 자리에서 반올림한 
각각의 값을 출력

# salary, 소수점첫째, 둘째, 정수첫째..

SELECT salary, salary/30, ROUND(salary/30,1),
       ROUND(salary/30,2), ROUND(salary/30,-1)
 FROM employees;
 
SELECT salary, salary/30, TRUNCATE(salary/30,1),
       TRUNCATE(salary/30,2), TRUNCATE(salary/30,-1)
 FROM employees;
...
SELECT SYSDATE(), hire_date,
       TIMESTAMPDIFF(YEAR, hire_date, SYSDATE())
 FROM employees
 WHERE department_id = 100; 
...
SELECT hire_date, PERIOD_ADD("2021-07-23", 5) 
  FROM employees
  LIMIT 3;
...
SELECT hire_date,
       DATE_ADD(hire_date, INTERVAL 6 DAY),
       DATE_ADD(hire_date, INTERVAL -6 DAY) 
FROM employees
WHERE employee_id BETWEEN 100 AND 106; 
...
commission_pct가 NULL(0) 일 때에는 1로 치환하여 
commission_pct를 곱한 
결과를 출력 (employee_id, salary, commission_pct, 
              salary_new)  
...
[예제 4-14] 조건논리 처리 
employees테이블에서first_name, last_name, department_id,
salary를출력하되 
department_id 가 60인 경우에는 급여를 10% 인상한 값을 
   계산하여 출력하고,
나머지 경우에는 원래의 값을 출력하세요.  
그리고 department_id가 60인 경우에는
'10%인상'을 출력하고 나머지 경우네는
 '미인상'을 출력하세요. 
...
employees 테이블에서  employee_id, 
first_name, last_name, salary를 출력하되, 
salary가 9000이상이면 '상위급여', 
6000과 8999 사이면 '중위급여', 
그 외에는 '하위급여'  를 출력하세요
단 job_id가 IT_PROG 직원대상.  
...
RANK, DENSE_RANK, ROW_NUMBER함수를 각각 이용해 
employees 테이블의 
...
[예제 4-19] SUM, AVG 함수
employees테이블에서salary의 합계와 평균을 구해 보세요. 
또한 AVG 함수를 사용하지않고 salary의 평균을 구해보세요 
...
[예제 4-20] MAX, MIN 함수 
employees테이블에서salary의 최댓값과 최솟값, 
first_name의 최댓값과 최솟값을 출력해 보세요 
...
employees테이블과 departments 테이블과 
locations 테이블을 조인하여 
각 직원이 어느 부서에 속하는지와 
부서의 소재지가 어디인지 조회하세요. 
...
employees테이블과 departments테이블을 department_id로 
외부 조인하여 department_id가 NULL값인 
Kimberely GRANT도 함께 출력 
