...
employees 테이블에서 employee_id, first_name, 
last_name을 employee_id를 기준으로 내림차순으로 출력하세요. 

[예제 3-4]  결과에서 중복값 제외 
employees 테이블에서 중복 값이 생기지 않도록 job_id를 출력하세요. 

[예제 3-5] 별칭 사용
employees 테이블에서 employee_id는  사원번호 , first_name은 이름, last_name은 성으로 출력하세요 

[예제 3-6] 연결 연산자
employees 테이블에서 employee_id를 출력하고 first_name과 last_name을 붙여서 출력하세요. 

[예제 3-7] 연결 연산자 
employees 테이블에서 employee_id를 출력하고 first_name과 last_name을 붙여서 출력, email에 “@company.com”을 
붙여서 출력 하세요 

[예제 3-8] 산술처리
employees 테이블에서 employee_id, salary, salary에 500을 더한값, 100을 뺀값, 10%를 추가해서 2로 나눈 값을 출력 

[예제 3-9] 산술처리
employees 테이블에서 employee_id를 사원번호, salary를 급여, salary에 500을 더한값을 추가급여, 100을 뺀값을 인하급여, 
...
[예제 3-14] WHERE 조건절 IN  연산자employees 테이블에서 
salary가 10000, 17000, 24000인 직원 정보를 출력

[예제 3-15] WHERE 조건절 LIKE  연산자
employees 테이블에서 job_id값이 AD로 시작하는 모든 데이터를 조회

[예제 3-16] WHERE 조건절 LIKE  연산자
employees 테이블에서 job_id가 AD로 시작하면서 5자인 데이터 
값을 갖는 직원 정보를 조회

employees 테이블에서 first_name이 Le로 시작하는 
모든 데이터를 조회

employees 테이블에서 first_name에 ame가 들어있는 
모든 데이터를 조회

employees 테이블에서 first_name이  in으로 끝나는 
모든 데이터를 조회
...
employees 테이블에서 employee_id가 105이 아닌 직원을 조회
...
employees 테이블에서 first_name에 대해 12자리의 문자열 자리를 만들되 first_name의 데이터 값이 12자리보다 작으면 
왼쪽에서 부터 *를 채워서 출력

[예제 4-5] 숫자 타입 함수(ltrim, rtrim) 
employees 테이블에서 job_id의 데이터 값에 대해 왼쪽 방향부터 'F' 문자를 만나면 삭제하고 또 오른쪽 방향부터 'T' 문자를 만나면 삭제


[예제 4-6] 숫자 타입 함수(round) 
employees 테이블에서 salary를 30일로 나눈 후 나눈 값의 소수점 첫째 자리, 소수점 둘째 자리, 정수 첫째 자리에서 
반올림한 각각의 값을 출력


[예제 4-7] 숫자 절삭하기
employees테이블에서 salary를 30일로 나누고 나눈 값의 소수점 첫째 자리, 소수점 둘째 자리, 정수 첫째 자리에서 
절삭하여 출력하세요. 
...
employees 테이블에서 department_id가 100인 직원에 대해 오늘 날짜, hire_date, 오늘날짜와 hire_date사이의 개월수를 출력

[예제 4-9] 월에 날짜 더하기 
employees 테이블에서 department_id가 100과 106사이인 직원의 hire_date에 3개월을 더한 값, hire_date에 3개월을 뺀 값을 출력
...
commission_pct가 NULL일 때에는 1로 치환하여 commission_pct를 곱한 결과를 출력
(employee_id, salary, commission_pct, salary_new) 

[예제 4-14] 조건논리 처리 
employees테이블에서first_name, last_name, department_id, salary를 출력하되 department_id 가 
60인 경우에는 급여를 10% 인상한 값을 계산하여 출력하고,
나머지 경우에는 원래의 값을 출력하세요. 
그리고 department_id가 60인 경우에는 '10%인상'을 출력하고
나머지 경우네는 '미인상'을 출력하세요. 

[예제 4-15] 복잡한 조건 논리 처리
employees 테이블에서 job_id가 IT_PROG라면 employee_id, first_name, last_name, salary를 출력하되, 
salary가 9000이상이면 '상위급여', 6000과 8999 사이면 '중위급여', 그 외에는 '하위급여'를 출력하세요 

[예제 4-16] 데이터값에 순위 매기기
RANK, DENSE_RANK, ROW_NUMBER함수를 각각 이용해 employees 테이블의 salary값이 높은 순서대로 순위를 매겨 출력하라. 
...
employees테이블에서 employee_id가 10 이상인 직원에 대해,
job_id별로 그룹화하여 job_id별 총 급여와 job_id별 평균급여를 구하되, job_id별 총 급여가 30000보다 큰 값만 출력하세요. 출력결과는 
job_id별 총급여를 기준으로 내림차순 정렬하세요. 


[5장] ERD 데이터베이스 개체 설계도 

5.2 employee_id가 101인 직원의 first_name, last_name, employee_id, start_date, end_date, job_id, 
department_id를 출력

[6장] 조인과 집합 : 여러 개의 테이블 연결하기 

[예제 6-1] 동등조인 : 똑같은 데이터끼리 연결하기
employees테이블과 departments 테이블과 locations 테이블을 조인하여 각 직원이 어느 부서에 속하는지와 부서의 소재지가 어디인지 
조회하세요. 

[예제 6-2] 외부조인 : 모든 데이터를 연결하기
employees테이블과 departments테이블을 department_id로 외부 조인하여 department_id가 NULL값인 Kimberely GRANT도 출력 

[예제 6-3] 자체 조인 : 자기 자신의 데이터와 연결하기
employees테이블을 자체 조인하여 직원별 담당 매니저가 누구인지 조회

[예제 6-4] 집합 연산자 : 집합으로 연결하기
employees테이블의 department_id집합과 departments테이블의 department_id집합을 UNION 연산자를 이용해 합쳐 보세요. 
(중복값이 없음) 
