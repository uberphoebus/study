...
# 4. 가로넓이 : 이미 넓이 지정 (원본유지, 1024, 800, 640)
...
btn_add_file = Button(
    file_frame, text='파일 추가', padx=5, pady=5, width=12,
    command=add_file
)
btn_add_file.pack(side='left')

btn_del_file = Button(
    file_frame, text='선택 삭제', padx=5, pady=5, width=12, 
    command=del_file
)
...
cmb_space = ttk.Combobox(
    frame_option, state='readonly', values=opt_space, width=10
    )
cmb_space.current(0)
...
progress_bar = ttk.Progressbar(
    frame_progress, maximum=100, variable=p_var
    )
...
btn_close = Button(
    frame_run, padx=5, pady=5, text='닫기', 
    width=12, command=root.quit
    )
btn_close.pack(side='right', padx=5, pady=5)

btn_start = Button(
    frame_run, padx=5, pady=5, text='시작', width=12, 
    command=start
    )
