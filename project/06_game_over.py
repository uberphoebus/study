...
# 게임 종료 : TimeOut, Mission Complete, Game Over
...
while running:
    ...
    character_rect.left, character_rect.top = character_x_pos, character_y_pos
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        ball_rect.left, ball_rect.top = ball_pos_x, ball_pos_y
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            weapon_pos_x, weapon_pos_y = weapon_val[0], weapon_val[1]
            
            # 무기 rect 정보 업데이트
            weapon_rect = weapon.get_rect()
            weapon_rect.left, weapon_rect.top = weapon_pos_x, weapon_pos_y
            
            # 충돌 체크
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx # 해당 무기 없애기 위한 값 설정
                ball_to_remove = ball_idx # 해당 공 없애시 위한 값 설정
                
                # 가장 작은 크기의 공이 아니면 다음 단계의 공으로 나눠짐
                if ball_img_idx < 3:
                    # 현재 공 크기 정보를 가져 옴
                    ball_width, ball_height = ball_rect.size[0], ball_rect.size[1]
                    ...
                    balls.append(
                        {
                            'pos_x': ball_pos_x + (ball_width / 2) - (small_ball_width / 2), # 공의 좌표
            ...
            continue
        break # 안쪽 for문에서 break를 만나면 2중 for문 탈출
