...
def pykrx_openapi_df(limit_cnt=15):
    ...
    krx_df['변동률'] = krx_df.groupby('종목코드')['종가'].apply(myfunc)

    df_list = krx_df.sort_values('날짜').groupby('종목코드')['변동률'].apply(lambda x: list(x))
    mg = krx_df[krx_df['날짜'] == krx_df['날짜'].max()]

    krx_df = pd.merge(mg, df_list, how='left', left_on='종목코드', right_on=df_list.index)
